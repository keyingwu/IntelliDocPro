# IntelliDocPro：schema 驱动的文档抽取后端设计
...
IntelliDocPro 是一个文档处理平台（现有产物是一个纯 UI 原型 `IntelliDocPro-standalone.html`，无真实解析逻辑）。第一步是把核心能力抽成一个可复用的中间层:
...
```
backend/
  pyproject.toml            # 包名 intellidocpro, 可独立安装
  src/intellidocpro/
...
- SDK: `anthropic`，默认模型 `claude-opus-4-8`（`INTELLIDOCPRO_CLAUDE_MODEL` 可覆盖）
...
- OpenAI 原生: `OPENAI_API_KEY`，默认模型 `gpt-5.6-terra`（2026-07 最新 GPT-5.6 家族的平衡档，支持 PDF 输入 + structured outputs；`INTELLIDOCPRO_OPENAI_MODEL` 可覆盖，如 `gpt-5.6-sol` 冲精度、`gpt-5.6-luna` 降成本）
...
class EngineError(IntelliDocProError): ...   # 包 API 异常, 附引擎名/request id
class EngineNotConfigured(IntelliDocProError): ...  # 缺环境变量
```
...
| `INTELLIDOCPRO_CLAUDE_MODEL` | 可选，默认 `claude-opus-4-8` |
| `OPENAI_API_KEY` | openai 引擎 |
| `INTELLIDOCPRO_OPENAI_MODEL` | 可选，默认 `gpt-5.6-terra` |
