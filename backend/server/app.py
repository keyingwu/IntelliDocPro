"""HTTP wrapper around intellidocpro. Run with: uvicorn server.app:app"""
...
from intellidocpro.errors import (
    DocumentTooLarge,
    EngineError,
    EngineNotConfigured,
...
app = FastAPI(title="IntelliDocPro API", version="0.1.0")
...
@app.exception_handler(intellidocpro.IntelliDocProError)
async def intellidocpro_error_handler(request, exc: intellidocpro.IntelliDocProError):
    status = next(
...
@app.get("/health")
def health():
    return {"status": "ok", "engines": intellidocpro.available_engines()}
...
def models():
    ...
    from intellidocpro.pricing import PRICES_AS_OF, lookup_price
...
async def extract(
    ...
    doc = intellidocpro.Document.from_bytes(data, filename=file.filename or "document")
    result = intellidocpro.extract(doc, schema_dict, engine=engine, model=model)
...
async def compare(
    ...
    data = await file.read()
    doc = intellidocpro.Document.from_bytes(data, filename=file.filename or "document")
    entries = intellidocpro.compare_engines(doc, schema_dict, candidates=candidate_list)
    return {
        "prices_as_of": intellidocpro.PRICES_AS_OF,
...
async def bulk_start(
    ...
    try:
        ...
        raise SchemaValidationError(f"schema is not valid JSON: {exc}") from exc
    schema = intellidocpro.ExtractionSchema.coerce(schema_dict)
    intellidocpro.get_engine(engine, model=model)  # surface config errors as 4xx/503 now
    ...
    jobs.publish(
        ...
        intellidocpro.BulkReport(
            ...
            model=model,
            total=len(documents),
            entries=[intellidocpro.BulkFileEntry(filename=name) for name, _ in documents],
        ),
    )

    def run() -> None:
        intellidocpro.bulk_extract(
...
async def suggest(
    ...
    data = await file.read()
    doc = intellidocpro.Document.from_bytes(data, filename=file.filename or "document")
    schema = intellidocpro.suggest_schema(doc, engine=engine, model=model)
...
async def refine_schema(
    ...
    data = await file.read()
    doc = intellidocpro.Document.from_bytes(data, filename=file.filename or "document")
    result = intellidocpro.refine_schema(
        doc,
        schema_dict,
        instruction,
