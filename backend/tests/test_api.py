"""Tests for the top-level intellidocpro.extract / suggest_schema API."""

import pytest

import intellidocpro
from intellidocpro.engines.openai import OpenAIExtractor
from intellidocpro.errors import SchemaValidationError, UnknownEngine
...
def test_extract_with_bytes_and_dict_schema(fake_openai):
    result = intellidocpro.extract(PDF, {"fields": [{"name": "Lieferant"}]})
...
def test_extract_with_path(tmp_path, fake_openai):
    p = tmp_path / "doc.pdf"
    p.write_bytes(PDF)
    result = intellidocpro.extract(p, ExtractionSchema(fields=[FieldSpec(name="Lieferant")]))
    assert result.values[0].value == "Meridian GmbH"


def test_extract_bad_schema():
    with pytest.raises(SchemaValidationError):
        intellidocpro.extract(PDF, {"fields": []})


def test_extract_unknown_engine():
    with pytest.raises(UnknownEngine):
        intellidocpro.extract(PDF, {"fields": [{"name": "a"}]}, engine="nope")
...
def test_suggest_schema(monkeypatch):
    ...
    schema = intellidocpro.suggest_schema(PDF)
