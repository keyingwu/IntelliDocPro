...
def test_agent_crud(client):
    ...
    assert client.get("/agents").json()[0]["id"] == a["id"]
    assert client.get(f"/agents/{a['id']}").json()["name"] == "Invoices"
...
def test_create_with_unknown_engine_rejected(client):
    resp = client.post(
        "/agents", json={"name": "x", "engine": "nope", "schema": SCHEMA}
    )
...
def test_create_with_invalid_schema_rejected(client):
    resp = client.post(
        "/agents", json={"name": "x", "engine": "claude", "schema": {"fields": []}}
    )
    assert resp.status_code == 422
...
def fake_bulk(monkeypatch):
    monkeypatch.setattr(intellidocpro, "bulk_extract", _fake_bulk)
    monkeypatch.setattr(intellidocpro, "get_engine", lambda *a, **k: object())


def _wait_results(client, agent_id, n, timeout=3.0):
    deadline = time.time() + timeout
    while time.time() < deadline:
        rows = client.get(f"/agents/{agent_id}/results").json()
        if len(rows) >= n:
            return rows
        time.sleep(0.02)
...
def test_agent_bulk_persists_results(client, fake_bulk):
    a = _create(client)
    resp = client.post(
        f"/agents/{a['id']}/bulk",
        files=[
    ...
    listed = client.get(f"/agents/{a['id']}").json()
    assert listed["doc_count"] == 2
    assert listed["review_count"] == 2
    assert listed["total_cost_usd"] == pytest.approx(0.04)

    review = client.get(f"/agents/{a['id']}/results", params={"filter": "review"}).json()
    assert len(review) == 2


def test_bulk_unknown_agent_404(client, fake_bulk):
    resp = client.post(
        "/bulk-nonexistent" if False else "/agents/nope/bulk",
...
def test_export_xlsx(client, fake_bulk):
    ...
    resp = client.get(f"/agents/{a['id']}/export.xlsx")
...
def test_export_unknown_agent_404(client):
    assert client.get("/agents/nope/export.xlsx").status_code == 404
...
def test_sample_unknown_agent_404(client):
    resp = client.put(
        "/agents/nope/sample", files={"file": ("s.pdf", PDF, "application/pdf")}
    )
    assert resp.status_code == 404
