...
from intellidocpro.errors import EngineError, EngineNotConfigured
from intellidocpro.result import ExtractionResult, FieldValue
from intellidocpro.schema import ExtractionSchema, FieldSpec
...
def test_health(client, monkeypatch):
    monkeypatch.setattr(
        app_module.intellidocpro, "available_engines", lambda: {"claude": True, "openai": False, "azure_openai": False}
    )
...
def test_extract_ok(client, monkeypatch):
    ...
    monkeypatch.setattr(app_module.intellidocpro, "extract", fake_extract)
    resp = client.post(
        "/extract",
...
def test_engine_not_configured_maps_503(client, monkeypatch):
    ...
    monkeypatch.setattr(app_module.intellidocpro, "extract", boom)
    resp = client.post(
        "/extract",
        files={"file": ("a.pdf", PDF, "application/pdf")},
        data={"schema": SCHEMA_JSON},
...
def test_compare_ok(client, monkeypatch):
    from intellidocpro.compare import CompareEntry
    from intellidocpro.pricing import cost_of
    ...
    monkeypatch.setattr(app_module.intellidocpro, "compare_engines", fake_compare)
    resp = client.post(
        "/compare",
        files={"file": ("a.pdf", PDF, "application/pdf")},
...
def test_bulk_job_lifecycle(client, monkeypatch):
    ...
    monkeypatch.setattr(app_module.intellidocpro, "bulk_extract", fake_bulk_extract)
    monkeypatch.setattr(app_module.intellidocpro, "get_engine", lambda *a, **k: object())
...
def test_bulk_engine_not_configured_rejected_upfront(client, monkeypatch):
    ...
    monkeypatch.setattr(app_module.intellidocpro, "get_engine", boom)
    resp = client.post(
        "/bulk",
...
def test_suggest_ok(client, monkeypatch):
    ...
    monkeypatch.setattr(
        app_module.intellidocpro, "suggest_schema", lambda doc, engine="claude", model=None: fake_schema
    )
...
def test_refine_ok(client, monkeypatch):
    ...
    def fake_refine(doc, schema, instruction, history=None, engine="claude", model=None):
        ...
        return intellidocpro.SchemaRefinement(
    ...
    monkeypatch.setattr(app_module.intellidocpro, "refine_schema", fake_refine)
    resp = client.post(
        "/schema/refine",
        files={"file": ("a.pdf", PDF, "application/pdf")},
        data={
