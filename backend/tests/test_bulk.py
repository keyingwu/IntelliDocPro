...
import intellidocpro.bulk as bulk_module
from intellidocpro.bulk import bulk_extract
...
def test_bulk_empty_documents_rejected(fake_engine):
    with pytest.raises(IntelliDocProError, match="no documents"):
        bulk_extract([], SCHEMA)
...
def test_bulk_bad_schema_raises_before_any_call(fake_engine):
    with pytest.raises(IntelliDocProError):
